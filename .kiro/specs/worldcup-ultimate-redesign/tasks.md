...
- [ ] 1. Foundation: theme tokens, motion system, and glass card base
  - [ ] 1.1 Create `app/ui/design/hud_theme.py` token system
  ...
  - [ ]* 1.2 Write unit tests for token helpers
    - Test `rgba()`/`mix()` output formatting and clamping; assert gradient ramp values match `#06111A`/`#0A1B28`/`#0F2A1F`
    - _Requirements: 16.2, 20.1_

  - [ ] 1.3 Create `app/ui/design/motion_system.py` built on `AnimationManager`/`tokens`
  ...
  - [ ] 1.6 Create `app/ui/widgets/glass_card.py` (`GlassCard`) replacing `misc.Card`
  ...
  - [ ]* 1.7 Write property test for hover lift magnitude
...
- [ ] 2. Rendering core: Stage Compositor (GPU + CPU fallback) and night-stadium backdrop
  - [ ] 2.1 Define the Stage Compositor public API and backend factory
  ...
  - [ ] 2.2 Implement the 5-layer night-stadium backdrop (both backends)
  ...
  - [ ]* 2.3 Write unit tests for ambient layer opacity bands and static trophy
  ...
  - [ ] 2.4 Implement per-frame render loop and ~60 FPS ambient throttle
  ...
  - [ ] 2.5 Wire Stage Compositor into `MainWindow` and remove old backdrops
  ...
  - [ ]* 2.6 Write integration test for backend selection/equivalence
...
- [ ] 3. Particle Engine and Floodlight Sweep (single-FrameClock driven)
  - [ ] 3.1 Implement the Particle Engine
  ...
  - [ ]* 3.2 Write property tests for particle bounds
  ...
  - [ ] 3.3 Implement the Floodlight Sweep
...
- [ ] 4. Checkpoint — foundation and rendering core
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 5. Reusable effect widgets and animated chart system
  - [ ] 5.1 Implement `CountUpNumber` (`app/ui/widgets/fx/count_up.py`)
  ...
  - [ ]* 5.2 Write property test for count-up exact settle
  ...
  - [ ] 5.3 Implement `FloatingFlag` (`app/ui/widgets/fx/floating_flag.py`)
  ...
  - [ ] 5.4 Implement `MouseTrailOverlay` (`app/ui/widgets/fx/mouse_trail.py`)
  ...
  - [ ]* 5.7 Write property test for chart reveal settling
...
- [ ] 6. Global chrome: Navigation Rail, Top HUD Bar, Sub-Header
  - [ ] 6.1 Rebuild the Navigation Rail on `GlassCard`/`hud_theme`
  ...
  - [ ] 6.4 Build the Sub-Header and Live Connection Pill
    - "2026 美加墨世界杯 · 实时数据总览"; "OVERVIEW · 数据来源：懂球帝公开接口"; right-aligned green "实时数据已连接" pill with status dot
    - _Requirements: 4.1, 4.2, 4.3_

- [ ] 7. Overview Page composition with weighted IA grid
  - [ ] 7.1 Build `HomePage` weighted layout skeleton
  ...
  - [ ]* 7.2 Write unit test for IA stretch ratios
...
- [ ] 8. Hero Match Card
  - [ ] 8.1 Implement `HeroMatchCard` layout and labels
  ...
  - [ ] 8.2 Implement the live countdown (single per-second timer)
  ...
  - [ ]* 8.3 Write property test for countdown monotonicity
  ...
  - [ ] 8.4 Implement the win-probability split bar
  ...
  - [ ]* 8.5 Write property test for win-probability split integrity
  ...
  - [ ] 8.6 Render per-team Elo / FIFA rank / world rank with placeholders
    - Show Elo, FIFA ranking, world ranking per team; render "—" for any missing value
    - _Requirements: 8.1, 8.2_

  - [ ]* 8.7 Write unit test for ranking placeholder fallback
    - Assert missing rating/rank values render "—"
    - _Requirements: 8.2_

- [ ] 9. Checkpoint — chrome, Overview shell, and Hero
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 10. Standings Panel (v2)
  - [ ] 10.1 Implement standings helper functions and form/qual/sparkline widgets
    - Add `rank_delta_glyph(delta)`, `FormPills`, `QualBar`, `MiniSparkline` to `app/ui/widgets/fx/`
    - _Requirements: 10.3, 10.5, 10.7_

  - [ ]* 10.2 Write property tests for standings helpers
  ...
  - [ ] 10.3 Implement `StandingsTable` panel composition
  ...
  - [ ]* 10.4 Write unit test for group selector behavior
    - Assert A active initially; selecting a tab marks it active and renders that group's standings
    - _Requirements: 9.3, 9.4_

- [ ] 11. Stat Strip
  - [ ] 11.1 Implement the Stat Strip row of six glass stat cards
  ...
  - [ ]* 11.2 Write unit test for Stat Strip composition
    - Assert exactly six cards in the specified order with the sampled headline/secondary/icon content
    - _Requirements: 11.1, 11.2_

- [ ] 12. Bottom row panels
  - [ ] 12.1 Implement the Today Matches Panel
  ...
  - [ ] 12.2 Implement `LiveMatchCenter` panel
  ...
  - [ ]* 12.3 Write tests for Live Match Center behaviors
  ...
  - [ ] 12.4 Implement the Top Scorers Panel
  ...
  - [ ] 12.5 Implement the Host Cities Panel
  ...
  - [ ] 12.6 Wire all bottom panels into the Overview Page bottom row
...
- [ ] 13. Checkpoint — full Overview composition renders end-to-end
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 14. First-open scene wiring and page transition
  - [ ] 14.1 Compose the first-open broadcast scene and navigation transition
...
- [ ] 15. Error handling and graceful degradation
  - [ ] 15.1 Implement data and rendering error handling
  ...
  - [ ]* 15.2 Write integration test for LOW_PERF and fetch-failure recovery
...
- [ ] 16. Performance pass and off-thread data audit
  - [ ] 16.1 Apply performance optimizations
  ...
  - [ ]* 16.2 Write tests for off-thread data and timer audit
...
- [ ] 17. Cleanup and migration finalization
  - [ ] 17.1 Remove replaced modules and migrate remaining pages
